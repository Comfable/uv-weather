function weatherOWM(latlong, citys, resolve) {
    ...
    owmAPIback = 'https://uvweather.herokuapp.com/https://api.openweathermap.org/data/2.5/weather?lat=' + lat + '&lon=' + lng + '&appid=6761dd7f8d0c3c216f9af6813064f867';
    fetch(owmAPIback)
        .then((resp) => resp.json())
        .then(function(resultOWM) {
            ...
            if (temperatureCbadge == '-0') {
                temperatureCbadge = 0
            };
            if (temperatureFbadge == '-0') {
                temperatureFbadge = 0
            };
